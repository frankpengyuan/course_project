...
def extfea(logcol):
	...
	timeslot_op1_num = [0 for i in range(op_clsnum)]
	timeslot_op2_num = [0 for i in range(op_clsnum)]
	timeslot_op3_num = [0 for i in range(op_clsnum)]
	...
	for l in args:
		...
		op_num[op_id]+=1
		logdate,logtime = l[3].split('T')
		#print float((todate(logdate)-start_date).days)
		if log_datespan == 0:
			pass
		else:
			temp_datalength = (float((todate(logdate)-start_date).days) / log_datespan)
				#print temp_datalength
			if temp_datalength < (0.3):
				#print temp_datalength
				timeslot_op1_num[op_id]+=1
			elif temp_datalength < (0.7):
				#print temp_datalength
				timeslot_op2_num[op_id]+=1
			else:
				#print temp_datalength
				timeslot_op3_num[op_id]+=1
	...
	for op_id in range(op_clsnum):
		feastr += " %d:%d" % (op_id+st_index, timeslot_op1_num[op_id])
	st_index += op_clsnum

	for op_id in range(op_clsnum):
		feastr += " %d:%d" % (op_id+st_index, timeslot_op2_num[op_id])
	st_index += op_clsnum

	for op_id in range(op_clsnum):
		feastr += " %d:%d" % (op_id+st_index, timeslot_op3_num[op_id])
	st_index += op_clsnum
...
if __name__=="__main__":
	...
	write_train("clean/truth_train.csv", feadic, "fea/train_fea2")
