...
def extfea(logcol):
	...
	for op_id in range(op_clsnum):
		feastr += ",%d" % (op_num[op_id])
	st_index += op_clsnum

	for op_id in range(op_clsnum):
		feastr += ",%d" % (timeslot_op1_num[op_id])
	st_index += op_clsnum

	for op_id in range(op_clsnum):
		feastr += ",%d" % (timeslot_op2_num[op_id])
	st_index += op_clsnum

	for op_id in range(op_clsnum):
		feastr += ",%d" % (timeslot_op3_num[op_id])
	st_index += op_clsnum
...
if __name__=="__main__":
	...
	write_train("clean/truth_train.csv", feadic, "fea/train_fea2.csv")
